...
// Check if this is a PDF generation request - DO THIS AT THE VERY TOP
...
function getUniqueValues($mysqli, $column, $table = 'equipment')
{
...
$locations_list = ['Mamburao', 'San Jose', 'Sablayan', 'Lubang'];
$equipment_data = [];
...
$all_equipment_stats = getEquipmentStats($mysqli, null, null, false);
$heavy_equipment_stats = getEquipmentStats($mysqli, null, 'Heavy Equipment', false);
$light_equipment_stats = getEquipmentStats($mysqli, null, 'Light Equipment', false);
...
function generatePDF($mysqli)
{
    ...
    if ($tab === 'pre-repair') {
        ...
        $summary_tables = [
            ['title' => 'ALL EQUIPMENT ALLOCATION', 'data' => getEquipmentStats($mysqli, null, null, false)],
            ['title' => 'HEAVY EQUIPMENT ALLOCATION', 'data' => getEquipmentStats($mysqli, null, 'Heavy Equipment', false)],
            ['title' => 'LIGHT EQUIPMENT ALLOCATION', 'data' => getEquipmentStats($mysqli, null, 'Light Equipment', false)]
        ];
    ...
    // Instead of multiple separate queries, use one comprehensive query:
    $query = "SELECT 
    e.location,
    e.type,
    ec.category_name,
    ...
    FROM equipment e
    LEFT JOIN equipment_category ec ON e.category_id = ec.id
    GROUP BY e.location, e.type, ec.category_name
    ORDER BY e.location, e.type, ec.category_name";
...
<div class="main-content-wrapper" style="overflow: ;">
    ...
    <!-- Your existing container with additional padding on the left -->
    <div class="container mx-auto px-6 py-8" style="max-width: 1400px;">
        ...
        <div id="pre-repair" class="tab-content <?php echo ($filters['tab'] === 'pre-repair' ? 'active' : ''); ?>">
            ...
            <p id="clock" class="text-xs text-black-400"></p>
