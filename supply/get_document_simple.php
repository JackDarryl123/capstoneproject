<?php
// get_document_simple.php - SIMPLIFIED VERSION FOR SUPPLY DIRECTORY
session_start();
...
if ($is_supply_view) {
    ...
    // Map status to the CURRENT index (the step that is "In Progress")
    // If status is 'pending', we are still on step 0.
    // If status is 'approved', step 1 is DONE, and we are now on step 2 (Complied).
    // If status is 'complied', step 2 is DONE, and we are now on step 3 (Received).
    $status_to_index = [
        'pending' => 0,
        'approved' => 2,
        'complied' => 3,
        'received' => 4 // 4 means all 4 steps (0-3) are done
    ];
} else {
            ...
            <div class="info-cards-row">
                ...
                <div class="info-card">
                    ...
                    <?php if (!empty($document['attached_file_path'])): 
                        $filePath = htmlspecialchars($document['attached_file_path']);
                        // Use BASE_URL for reliability
                        $displayFilePath = BASE_URL . '/' . $filePath;
                        ?>
                        <div class="file-action-wrapper">
                            <a href="<?= $displayFilePath ?>" class="file-link-btn"
