...
if (!isset($mysqli)) {
    error_log("side_request.php: \$mysqli is not set!");
    die("Database connection not available. Please refresh the page.");
}

if ($mysqli->connect_errno) {
    error_log("side_request.php: DB connection error: " . $mysqli->connect_error);
    die("Database connection failed. Please refresh the page.");
}

// Check and add columns only once per session (cache in session)
$schemaChecked = $_SESSION['documents_schema_checked'] ?? false;
if (!$schemaChecked) {
    ...
    $_SESSION['documents_schema_checked'] = true;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$insert_stmt) {
        ...
        echo "<script>alert('❌ Error preparing request. Please try again or contact support.'); window.history.back();</script>";
        exit;
    }
    ...
    if ($insert_stmt->execute()) {
        ...
        error_log("Document insert error: " . $insert_stmt->error);
        // Show user-friendly message
        echo "<script>alert('❌ Error submitting request. Please try again or contact support.'); window.history.back();</script>";
    }
                ...
                <div class="col-md-12">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <select id="statusFilter"
                            ...
                            style="height: 38px; width: auto; min-width: 140px; font-size: 0.9rem;"
                            onchange="applyFilters()">
                            <option value="All" <?= (!isset($_GET['status']) || $_GET['status'] == 'All') ? 'selected' : '' ?>>
                                All Status</option>
                            <option value="Pending" <?= (isset($_GET['status']) && $_GET['status'] == 'Pending') ? 'selected' : '' ?>>⏳ Pending</option>
                            <option value="Approved" <?= (isset($_GET['status']) && $_GET['status'] == 'Approved') ? 'selected' : '' ?>>✅ Approved</option>
                            <option value="Done" <?= (isset($_GET['status']) && $_GET['status'] == 'Done') ? 'selected' : '' ?>>Under-repair</option>
                        </select>
                    ...
                    if ($result && $result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            ...
                            $statusConfig = match ($status) {
                                ...
                                'Done' => ['bg' => 'bg-orange-50', 'text' => 'text-orange-700', 'dot' => 'bg-orange-400', 'icon' => 'bi-flag-fill'],
                            ...
                            if ($status !== 'Done') {
                                ...
                                echo "<i class='bi {$statusConfig['icon']}'></i>";
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="archivedSearch" class="form-control" placeholder="Search Property No.">
                    </div>
                </div>
                            ...
                            <div class="text-start mb-3 mt-2">
                                ...
                                <div class="input-group">
                                    <input type="text" name="prerepair_no" id="prerepair_no"
                                        ...
                                        placeholder="This field will be filled by the admin" form="requestForm"
                                        readonly>
                                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="text" name="carrying_amount" class="form-control"
                                                style="font-size: 0.85rem;" form="requestForm" readonly
                                                placeholder="This field will be filled by the admin">
                                ...
                                <div class="text-center"
                                    ...
                                    <div
                                        ...
                                        if (!empty($currentUser['signature'])): ?>
                                            <img src="/<?= htmlspecialchars($currentUser['signature']) ?>"
                                                alt="Signature"
