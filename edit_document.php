...
if (isset($_POST['mark_done'])) {
    $stmt = $mysqli->prepare("UPDATE documents SET status = 'Done' WHERE id = ?");
    $stmt->bind_param("i", $id);
    
    if ($stmt->execute()) {
        // Redirect after marking as done
        header("Location: http://localhost/PEPO/admin_dashboard.php?view=documents");
        exit(); // ✅ Always exit after a header redirect
    } else {
        echo "<div class='alert alert-danger text-center'>❌ Failed to mark document as Done.</div>";
    }
    ...
    if ($exists) {
        ...
        $stmt = $mysqli->prepare("
            UPDATE documents SET 
                ...
                officer_name=?, inspected_by=?, approved_by_pepo=?, witnessed_by=?, approved_by_gso=?,
                " . implode(', ', array_map(fn($i) => "material_$i=?, quantity_$i=?", range(1, 10))) . "
            WHERE id=?
        ");
        ...
        $stmt = $mysqli->prepare("INSERT INTO documents ($columns) VALUES ($placeholders)");
        $stmt->bind_param(str_repeat('s', count($params)), ...$params);
        $stmt->execute();

        echo "<div class='alert alert-success text-center'>✅ Pre-Repair Inspection Data added successfully!</div>";
    }
        ...
        <div class="card p-4 shadow-sm border-0">
            ...
            <section class="mb-4">
                ...
                <div class="text-start mb-3 mt-2">
                    ...
                    <input type="text" name="carrying_amount" class="form-control form-control-lg" value="<?= htmlspecialchars($doc['carrying_amount'] ?? '') ?>" readonly>
                </div>
                ...
                <div class="d-flex justify-content-center mt-3">
                    <div class="text-center" style="width:60%;">
                        <input type="text" name="officer_name" class="form-control form-control-lg text-center" value="<?= htmlspecialchars($doc['officer_name'] ?? '') ?>" readonly>
            ...
            <section class="mb-4">
                ...
                <div class="border p-3 mb-4 rounded bg-light" style="font-family: 'Times New Roman', Times, serif; font-size: 20px;">
                    <p class="mb-0">
                        I (Name)
                        <input type="text" 
                            name="inspector_name" 
                ...
                <div class="mb-3">
                    ...
                    <textarea name="defect" class="form-control form-control-lg" rows="3"><?= htmlspecialchars($doc['defect'] ?? '') ?></textarea>
                </div>

                <div class="mb-3">
                    <label class="fw-bold">Findings:</label>
                    <textarea name="findings" class="form-control form-control-lg" rows="3"><?= htmlspecialchars($doc['findings'] ?? '') ?></textarea>
                </div>

                <div class="mb-3">
                    <label class="fw-bold">Recommendation:</label><br>
                    <input type="radio" name="recommendation" value="For In-House Repair" <?= ($doc['recommendation'] ?? '') === 'For In-House Repair' ? 'checked' : '' ?>> For In-House Repair
                    <br>
                    <input type="radio" name="recommendation" value="For Outside Repair" <?= ($doc['recommendation'] ?? '') === 'For Outside Repair' ? 'checked' : '' ?>> For Outside Repair
                </div>
            </section>
                ...
                <table class="table table-bordered text-center align-middle">
                    ...
                    <tbody>
                        <?php for ($i = 1; $i <= 10; $i++): ?>
                            <tr>
                                <td><?= $i ?></td>
                                <td><input type="text" name="material_<?= $i ?>" class="form-control form-control-lg text-center" value="<?= htmlspecialchars($doc["material_$i"] ?? '') ?>"></td>
                                <td><input type="text" name="quantity_<?= $i ?>" class="form-control form-control-lg text-center" value="<?= htmlspecialchars($doc["quantity_$i"] ?? '') ?>"></td>
                            </tr>
                        <?php endfor; ?>
                    </tbody>
                </table>
            ...
            <div class="row text-center mt-5">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="inspected_by" class="form-control form-control-lg text-center" value="<?= htmlspecialchars($doc['inspected_by'] ?? '') ?>">
                    <small>Inspector</small>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="fw-bold">Approved:</label>
                    <input type="text" name="approved_by_pepo" class="form-control form-control-lg text-center" value="<?= htmlspecialchars($doc['approved_by_pepo'] ?? '') ?>">
                    <small>PGDH-PEPO</small>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="fw-bold">Witnessed:</label>
                    <input type="text" name="witnessed_by" class="form-control form-control-lg text-center" value="<?= htmlspecialchars($doc['witnessed_by'] ?? '') ?>">
                    <small>PGDH-PACCO</small>
                </div>
                <div class="col-md-6 mb-3">
                    <label class="fw-bold">Approved:</label>
                    <input type="text" name="approved_by_gso" class="form-control form-control-lg text-center" value="<?= htmlspecialchars($doc['approved_by_gso'] ?? '') ?>">
            ...
            <div class="text-center mt-5">
                ...
                <button type="submit" name="mark_done" class="btn btn-dark btn-lg px-5">
